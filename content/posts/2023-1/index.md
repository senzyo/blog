...
2. 在 `aria2.conf` 中的 `bt-tracker=` 后添加 [Tracker](https://github.com/XIU2/TrackersListCollection) 地址。
3. 按照配置文件, 创建需要的空文件 `aria2.session`, 下载 [dht.dat](https://github.com/P3TERX/aria2.conf/blob/master/dht.dat) 和 [dht6.dat](https://github.com/P3TERX/aria2.conf/blob/master/dht.dat)。
4. 修改 `aria2.conf` 中, 以上各文件的引用路径。
5. Windows 中, 我一般将 aria2 本体和配置文件放在一起, 比如 `D:\Aria2`。
6. Linux 中, 通过 pacman 安装的 aria2, 其路径是 `/usr/bin/aria2c`, 配置文件我一般放在 `/etc/aria2/` 中。
