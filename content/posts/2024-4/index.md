...
```batch
@echo off
DOSKEY ff-proxy-on=echo user_pref^("network.proxy.type", 1^); ^> "%APPDATA%\Mozilla\Firefox\Profiles\xxx\user.js" $t echo Proxy for Firefox enabled. Restart Firefox to apply.
DOSKEY ff-proxy-off=echo user_pref^("network.proxy.type", 0^); ^> "%APPDATA%\Mozilla\Firefox\Profiles\xxx\user.js" $t echo Proxy for Firefox disabled. Restart Firefox to apply.
```
...
```batch
@echo off
DOSKEY ff-proxy-on=xcopy /y "%APPDATA%\Mozilla\Firefox\Profiles\xxx\ff-proxy-on.js" "%APPDATA%\Mozilla\Firefox\Profiles\xxx\user.js" $t echo Proxy for Firefox enabled. Restart Firefox to apply.
DOSKEY ff-proxy-off=echo user_pref^("network.proxy.type", 0^); ^> "%APPDATA%\Mozilla\Firefox\Profiles\xxx\user.js" $t echo Proxy for Firefox disabled. Restart Firefox to apply.
```
...
'@ | Out-File -FilePath "$env:APPDATA\Mozilla\Firefox\Profiles\xxx\user.js" -Encoding UTF8
    Write-Host "Proxy for Firefox enabled. Restart Firefox to apply."
}

function ff-proxy-off {
    'user_pref("network.proxy.type", 0);' | Out-File -FilePath "$env:APPDATA\Mozilla\Firefox\Profiles\xxx\user.js" -Encoding UTF8
    Write-Host "Proxy for Firefox disabled. Restart Firefox to apply."
}
```
