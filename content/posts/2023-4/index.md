...
编辑 `/etc/makepkg.conf`, 找到类似 `OPTIONS=(strip docs !libtool !staticlibs emptydirs zipman purge !debug lto)`, 在 `debug` 前面加一个感叹号 `!`, 变成 `!debug`, 这样就避免安装像 `yay-debug` 这类 debug 包。
...
```bash
yay -S aria2 git google-chrome joplin-appimage kclock lunacy-bin telegram-desktop ttf-wps-fonts unrar unzip visual-studio-code-bin vlc wps-office wps-office-mui-zh-cn
```
...
fc-scan /usr/share/fonts/noto* --format='%{family}\n' | tr ',' '\n' | sort -u
```
...
为防止混乱, 我的 `~/.config/fontconfig/fonts.conf` 和 `/etc/fonts/local.conf` 中 **没有** 相同的配置项, 前者只保存来自 KDE 字体设置的配置信息, 后者保存自己高度自定义的配置信息。

- 参考 [Alias](https://wiki.archlinux.org/title/Font_configuration#Alias) 配置 fallback 字体顺序;
