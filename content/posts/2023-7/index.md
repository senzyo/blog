---
title: "Clash Core with Web Dashboard on Windows"
...
- Dashboard: https://github.com/MetaCubeX/metacubexd/releases/latest, 将 Dashboard 的文件夹重命名为 `MetaCubeXD`。

将 `ClashMetaCore.exe` 和 `MetaCubeXD` 文件夹一起放入 Clash 工作目录中, 比如 `D:\Clash Meta`。
...
1. 将以下内容保存为 `Clash Meta Core Service.xml`, 和 `winsw.exe` 一起放入 Clash 工作目录中。
    ...
    <service>
        ...
        <name>Clash Meta Core Service</name>
    ...
    注意, `<id>` 指 `Name`, `<name>` 指 `DisplayName`。
...
防火墙入站规则放行 Clash Core 程序和 `mixed-port` 端口的 TCP 和 UDP 流量。
...
为 `.ps1` 文件 (PowerShell 脚本文件) 创建快捷方式, 然后右击文件, “属性→快捷方式→目标”, 在文件路径前添加 `powershell.exe -ExecutionPolicy Bypass -File `, 这样以后只需点击该快捷方式即可。

不要忘记设置脚本的权限, 让普通用户仅限读取和执行。

此外, 因为管理任务计划程序和 Windows 服务需要管理员权限, 要让 `.ps1` 文件以管理员身份运行, 须在“属性→快捷方式→高级”中勾选“用管理员身份运行”, **或者** 在 `.ps1` 文件开头添加自动提权代码: 
...
- Dashboard: https://github.com/haishanh/yacd/releases/latest, 将 Dashboard 文件夹重命名为 `yacd`

将 `ClashPremiumCore.exe`, `wintun.dll` 文件和 `yacd` 文件夹一起放入 Clash 工作目录中, 比如 `D:\Clash Premium`。
...
参考微软官方文档 [Set-ExecutionPolicy](https://learn.microsoft.com/zh-cn/powershell/module/microsoft.powershell.security/set-executionpolicy), 以管理员身份运行 PowerShell, 运行以下命令: 
