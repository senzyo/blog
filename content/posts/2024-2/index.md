...
url_tpl="https://raw.githubusercontent.com/senzyo/sing-box-template/normal/$inbound/dot/8.8.8.8/ghproxy.net/config.json"  # 配置所用模板的地址
...
if [ $substate == "running" ]; then
    if [[ $inbound == "mixed" ]]; then
        ...
        echo -e "KDE proxy \e[1;32menabled\e[0m."
    elif [[ $inbound == "tun" ]]; then
        echo -e "Use \e[1;34mtun\e[0m mode."
        kwriteconfig6 --file $kioslaverc --group "Proxy Settings" --key "ProxyType" 0
        echo -e "KDE proxy \e[1;31mdisabled\e[0m."
    fi
fi
```
...
function sing-box-restart() {
    ...
    if [ $? -ne 0 ]; then
        echo -e "File \e[1;33m$dir_config/config.json\e[0m is invalid."
        sleep 2
        exit 1
    fi
...
function sing-box-switch() {
    ...
    if [ $? -ne 0 ]; then
        echo -e "File \e[1;33m$dir_config/config_$inbound.json\e[0m is invalid."
        sleep 2
        exit 1
    fi
